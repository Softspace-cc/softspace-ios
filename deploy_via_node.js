...
const tempDir = path.join(process.env.TEMP || 'C:\\Windows\\Temp', `softspace_temp_deploy_${Date.now()}`);
const zipFile = path.join(process.env.TEMP || 'C:\\Windows\\Temp', `softspace_deploy_${Date.now()}.zip`);
  ...
  try {
    ...
    conn.on('ready', () => {
      ...
      conn.sftp((err, sftp) => {
        ...
        console.log(`Uploading deploy package to /root/deploy.zip...`);
        let lastPercent = -1;
        sftp.fastPut(zipFile, '/root/deploy.zip', {
          ...
          if (err) throw err;
          console.log('Upload complete. Triggering update script on remote server...');
          
          const remoteCmd = `mkdir -p ${remotePath} && cp /root/deploy.zip ${remotePath}/ && cd ${remotePath} && unzip -o deploy.zip && chmod +x update.sh && ./update.sh`;
          ...
          conn.exec(remoteCmd, (err, stream) => {
            ...
            stream.on('close', (code, signal) => {
              ...
              console.log('DEPLOYMENT COMPLETE AND SERVER UPDATED!');
            }).on('data', (data) => {
