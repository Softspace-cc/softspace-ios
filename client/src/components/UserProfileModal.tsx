...
export default function UserProfileModal({
  ...
  const joinedDate = serverMemberInfo?.joinedAt 
    ? format(new Date(serverMemberInfo.joinedAt), 'PP', { locale: dateLocale })
    : null;

  const isActuallyMe = isMe || (me && me.id === user.id);
  const displayStatus = getDisplayStatus(user.status);
  const resolvedPlatform = isActuallyMe ? getClientPlatform() : (user.platform ?? 'web');
  const effectiveCanModerate = canModerate || me?.systemRole === 'CEO';
  const friendStatus = useMemo(() => {
    if (isActuallyMe) return null;
    if (!cachedFriends) return 'NONE';
    const f = cachedFriends.find((f: any) => f.user?.id === user.id || f.id === user.id || f.recipientId === user.id || f.requesterId === user.id);
    if (!f) return 'NONE';
    ...
    return 'NONE';
  }, [cachedFriends, user.id, isActuallyMe]);

  useEffect(() => {
    if (isOpen && user.id && !isActuallyMe) {
      setNote('');
      api(`/api/users/notes/${user.id}`, {}, token)
        .then(res => res.json())
        .then(data => {
      ...
      setMutualFriends([]);
      api(`/api/friends/mutual/${user.id}`, {}, token)
        .then(res => res.json())
        .then(data => {
  ...
  }, [isOpen, user.id, isActuallyMe, token]);

  const handleSaveNote = async (newNote: string) => {
    if (!token || isActuallyMe) return;
    setIsSavingNote(true);
    try {
      await api(`/api/users/notes/${user.id}`, {
        method: 'PUT',
  ...
  useEffect(() => {
    if (isOpen) {
      if (editStatus !== (user.status || 'online')) setEditStatus(user.status || 'online');
      if (editCustomStatus !== (user.customStatus || '')) setEditCustomStatus(user.customStatus || '');
    }
  }, [isOpen, user.status, user.customStatus]);
  ...
  const handleStartCall = async () => {
    ...
    const existingDm = cachedDms ? cachedDms.find((c: any) => !c.isGroup && c.members.some((m: any) => m.userId === user.id)) : null;
    if (existingDm) {
    ...
    if (dmChannelId) {
      onClose();
      navigate(`/app/dms/@${user.username}`);
  ...
  const handleModeration = async (updates: Record<string, any>) => {
    ...
    const res = await api(`/api/servers/${serverId}/members/${user.id}/moderation`, {
      method: 'PUT',
  ...
  const handleKick = async () => {
    if (!token || !serverId) return;
    const res = await api(`/api/servers/${serverId}/members/${user.id}`, { method: 'DELETE' }, token);
    if (!res.ok) {
  ...
  const handleBan = async () => {
    if (!token || !serverId) return;
    const res = await api(`/api/servers/${serverId}/bans/${user.id}`, {
      method: 'POST',
  ...
  const handlePlatformBan = async () => {
    if (!token || me?.systemRole !== 'CEO') return;
    ...
    const res = await api(`/api/users/${user.id}/platform-ban`, {
      method: 'POST',
      body: JSON.stringify({
  ...
  const moderationDialogTitle =
    ...
    moderationDialog === 'platform-ban' ? 'Platform Ban' :
    '';
        ...
        <div className="px-5 relative">
          ...
          <div className="pb-4">
            <h2 className="text-xl font-bold text-softspace-50 flex items-baseline gap-2">
              {displayName}
              {serverMemberInfo?.nickname && (
                <span className="text-sm font-normal text-softspace-400">
                  ({serverMemberInfo.nickname})
                </span>
              )}
            </h2>
            <div className="text-sm text-softspace-400">@{user.username}</div>

            {/* Custom Status (manual text only) */}
            {isManualCustomStatus(user.customStatus) && !isEditingStatus && (
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {user.status && (
                      <span className="text-xs bg-softspace-800 text-softspace-300 px-2 py-0.5 rounded font-medium capitalize flex items-center gap-1.5">
                        <StatusIndicator status={user.status} platform={resolvedPlatform} />
                        {displayStatus}
                      </span>
                    )}
                  </div>
          ...
          <div className="py-4 space-y-4 px-5">
            {profileTab === 'activity' ? (
              <ActivityPanel activitiesRaw={user.activities} />
            ) : profileTab === 'mutual-friends' ? (
            ...
            {/* Platform Moderation */}
            {!isActuallyMe && me?.systemRole === 'CEO' && (
              <div className="bg-softspace-950/40 p-3 rounded-xl border border-red-900/30">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-[11px] font-bold text-red-400 uppercase tracking-wider flex items-center gap-1.5">
                    <ShieldAlert size={12} /> Platform Moderation
                  </h3>
                  {user.platformBanCreatedAt && (
                  ...
                  {(moderationDialog === 'timeout' || moderationDialog === 'platform-ban') && (
                    <div>
                      ...
                      <input
                        ...
                        placeholder={moderationDialog === 'platform-ban' ? 'Leer = permanent' : 'z. B. 60'}
                      ...
                      {moderationDialog === 'platform-ban' && (
                        <div className="text-[11px] text-softspace-500 mt-1">
                          Leer lassen fuer permanenten Ban.
                        </div>
                      )}
                    </div>
                  )}
            ...
            {/* Roles */}
            {serverMemberInfo?.roles && serverMemberInfo.roles.length > 0 && (
              <div>
                <h3 className="text-[11px] font-bold text-softspace-500 uppercase tracking-wider mb-1.5">
                  {t('roles')}
                </h3>
                <div className="flex flex-wrap gap-1.5">
                  {serverMemberInfo.roles.map(role => (
                    <div
                      key={role.id}
